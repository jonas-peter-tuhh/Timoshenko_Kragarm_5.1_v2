...
Lb = 25#float(input('Länge des Kragarms [m]: '))
...
for i in range(LFS):
    ...
    Lq[i] = 25#float(input('Länge der ' + str(i + 1) + '. Streckenlast [m]: '))
...
##
torch.save(Timoshenko_Kragarm_5_1_v2.state_dict(), 'saved_data/Timoshenko_Kragarm_5.1_v2.pt')
...
plt.plot(x, (-1/120  *(x/4)**5 + 1/6 * 12.5 * (x/4)**3 - 41.67/2 * (x/4)**2)/EI + (1/6 * (x/4)**3 - 12.5*(x/4))/(K*A*G))
plt.grid()
...
plt.plot(x, (-1/24 * 0.25**5 *x**4/EI + 0.5*0.01116*x**2 - 0.1488*x))
plt.grid()
...
plt.plot(x, ( -0.25**5* 1/6 * x**3/EI + 0.0116*x - 0.1488))
plt.grid()
...
plt.plot(x, ((0.25**5 * 0.5 * x**2 - Q0[-1])/(K*A*G)))
